...
class TaskService:
    ...
    @classmethod
    def copy_task_events(cls, data: dict):
        ...
        source_events = db.session.query(TaskEvent).join(
            WorkPhase,
            and_(
        ...
        for work_phase in target_work_phases:
            if work_phase.task_added:
                raise BusinessError(f"Work phase: {work_phase.name} already has tasks in it")
