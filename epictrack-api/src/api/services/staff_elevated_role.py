...
class StaffElevatedRoleService:
    ...
    def find_by_staff_id(cls, staff_id):
        ...
        staff_elevated_role = StaffElevatedRoleModel.find_by_staff_id(staff_id)
        if staff_elevated_role:
            return staff_elevated_role
        raise ResourceNotFoundError(f"Staff Elevated Role with staff_id '{staff_id}' not found.")

    @classmethod
    def find_by_elevated_role_id(cls, elevated_role_id):
        """Find staff elevated roles by elevated role ID"""
        return StaffElevatedRoleModel.find_by_elevated_role_id(elevated_role_id)
