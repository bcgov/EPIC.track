...
class Work(BaseModelVersioned):
    ...
    @classmethod
    def fetch_all_works_by_work_status(
        cls,
        pagination_options: PaginationOptions,
        search_filters: StatusDashboardSearchOptions = None
    ) -> Tuple[List[Work], int]:
        """Fetch all active works."""
        query = cls.query.filter_by(is_deleted=False)
        query = cls.filter_by_status_search_criteria(query, search_filters)
        query = query.order_by(Work.start_date.desc())
    ...
    def filter_by_status_search_criteria(cls, query, search_filters: StatusDashboardSearchOptions):
        """Filter statuses by search criteria."""
        if not search_filters:
            return query
        ...
        query = cls._filter_by_project_is_active(query, search_filters.project_status)

        query = cls._filter_by_work_is_active(query, search_filters.work_status)
    ...
    @classmethod
    def _filter_by_project_is_active(cls, query, project_is_active):
        if project_is_active:
            subquery = exists().where(and_(Work.project_id == Project.id, Project.is_active.in_(project_is_active)))
            query = query.filter(subquery)
        return query

    @classmethod
    def _filter_by_work_is_active(cls, query, work_is_active):
        if work_is_active:
            query = query.filter(Work.is_active.in_(work_is_active))
        return query
