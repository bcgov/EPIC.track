...
def test_create_project(client, staff_admin_headers):
    """Test create new project."""
    url = urljoin(API_BASE_URL, "projects")
    response = client.post(url, json=TestProjectInfo.project1.value, headers=staff_admin_headers)
    assert response.status_code == HTTPStatus.CREATED
    assert "id" in response.json


def test_get_projects(client, staff_admin_headers):
    """Test get projects."""
    url = urljoin(API_BASE_URL, "projects")
    response = client.get(url, headers=staff_admin_headers)
    assert response.status_code == HTTPStatus.OK


def test_update_project(client, staff_admin_headers):
    """Test update project."""
    ...
    response = client.put(url, json=payload, headers=staff_admin_headers)

    assert response.status_code == HTTPStatus.OK
    assert response.json["name"] == "New Project Updated"


def test_delete_project(client, staff_admin_headers):
    """Test delete project."""
    project = factory_project_model()
    url = urljoin(API_BASE_URL, f'projects/{project.id}')
    client.delete(url, headers=staff_admin_headers)
    projects_url = urljoin(API_BASE_URL, "projects")
    response = client.get(projects_url, headers=staff_admin_headers)
    assert len(response.json) == 0


def test_project_detail(client, staff_admin_headers):
