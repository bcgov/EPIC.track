...
import { Container, FormLabel, Grid, MenuItem } from '@mui/material';
...
export default function ReportSelector({ ...props }) {
    ...
    const reportTypeOptions = REPORT_TYPES.map((p, index) => (<MenuItem key={index + 1} value={p.Value}>{p.Text}</MenuItem>))
    return (
        <>
            <Grid container>
                <Grid item sm={2}>
