...
class Milestone(BaseModel):
    ...
    milestone_type = relationship(
        "MilestoneType", foreign_keys=[milestone_type_id], lazy="select"
    )
    ...
    def find_by_phase_id(cls, _phase_id):
        ...
        milestones = (
            ...
            .filter_by(phase_id=_phase_id, kind=MilestoneKind.EVENT.value)
    ...
    def find_non_decision_by_phase_id(cls, _phase_id: int):
        ...
        milestones = (
            cls.query.filter_by(phase_id=_phase_id)
            .order_by(Milestone.sort_order.asc(), Milestone.id.asc())
            .all()
        )
    ...
    @classmethod
    def find_auto_by_phase_id(cls, _phase_id: int):
        """Find non decision by phase id."""
        milestones = (
            cls.query.filter_by(
                phase_id=_phase_id, kind=MilestoneKind.EVENT.value, auto=True
            )
            .order_by(Milestone.sort_order.asc(), Milestone.id.asc())
            .all()
        )
