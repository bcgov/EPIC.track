...
describe("EventCalendarContainer", () => {
  ...
  it("renders full icon and work legends when enabled", () => {
    setupIntercepts(eventCalendarBranchEndpoints);
    ...
    cy.contains("Icon Legend").should("exist");
    cy.contains("Work Legend").should("exist");
    cy.contains("Legend Work Alpha").should("exist");
  });

  it("shows milestone loading spinner when modal is open without work details", () => {
    setupIntercepts(eventCalendarBranchEndpoints);
    ...
    cy.wait("@getMilestones");
    cy.contains("button", "open-milestone-modal").click();

    cy.get(".MuiCircularProgress-root").should("exist");
    cy.contains("Icon Legend").should("not.exist");
    cy.contains("Work Legend").should("not.exist");
  });

  it("renders task side panel details after loading task event", () => {
    setupIntercepts(eventCalendarBranchEndpoints);
    ...
    cy.contains("button", "load-task-details").click();
    cy.wait("@getWorkById");
    cy.wait("@getTaskById");

    cy.contains("Current Month Task").should("exist");
    cy.contains("Legend Work Alpha").should("exist");
  });

  it("renders milestone side panel details after loading milestone event", () => {
    setupIntercepts(eventCalendarBranchEndpoints);
    ...
    cy.contains("button", "load-milestone-details").click();
    cy.wait("@getWorkById");
    cy.wait("@getWorkPhaseById");
    cy.wait("@getMilestoneById");

    cy.contains("Current Month Milestone").should("exist");
